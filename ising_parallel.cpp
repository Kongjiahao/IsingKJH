...
int main(int argc, char** argv)
{
    ...
    mt19937 gen(958431198 + world_rank); // Different seed for each process
    ...
    int N = 1e5 / world_size; // Divide N by number of processes
    ...
    for (tstar = tmax; tstar > tcrit_up; tstar -= deltat) {
        ...
        double global_m[DATA];
        MPI_Reduce(m, global_m, DATA, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);

        if (world_rank == 0) {
            for (int i = 0; i < DATA; i++) global_m[i] /= world_size;
            w_output(output, tstar, N * world_size, global_m);
            cout << tstar << endl;
        }
    }
